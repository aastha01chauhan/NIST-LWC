...
public class TestVectorISAP {
    ...
    public static int generateTestVectors() throws IOException {
        ...
        try {
            ...
            reader  = new BufferedReader(new FileReader("LWC_AEAD_KAT_128_128_REF-AUTHOR.txt"));
        } catch (IOException e) {
            System.err.println("Couldn't open WC_AEAD_KAT_128_128_REF-AUTHOR.txt");
        ...
        for (long mlen = 0; (mlen <= MAX_MESSAGE_LENGTH) && (ret_val == KAT_SUCCESS); mlen++) {
            for (long adlen = 0; adlen <= MAX_ASSOCIATED_DATA_LENGTH; adlen++) {
                ...
                if ((func_ret = ISAP128AEAD.crypto_aead_encrypt(
                ...
                if (! auth_CT.equals(myCTHex)) {
                   System.out.println("Author's CT and our CT is not equal: " + auth_CT + " " + myCTHex); 

                } 
             
                long[] _mlen2 = new long[msg.length];
                _mlen2[0] = msg.length;
                if ((func_ret = ISAP128AEAD.crypto_aead_decrypt(
                ...
                if (! auth_PT.equals(myPTHex)) {
                   System.out.println("Author's CT and our CT is not equal: " + auth_PT + " " + myPTHex);
