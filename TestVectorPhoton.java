...
public class TestVectorPhoton {
    ...
    public static int generateTestVectors() throws IOException {
        ...
        //int[] ct = new int[MAX_MESSAGE_LENGTH + CRYPTO_ABYTES];
        long clen;
        long mlen2;
        int count = 1;
        int func_ret;
        ...
        try {
            ...
            reader  = new BufferedReader(new FileReader("LWC_AEAD_KAT_128_128-AUTHOR.txt"));
        } catch (IOException e) {
            System.err.println("Couldn't open LWC_AEAD_KAT_128_128-AUTHOR.txt for read");
            return KAT_FILE_OPEN_ERROR;
        }

        for (long mlen = 0; (mlen <= MAX_MESSAGE_LENGTH) && (ret_val == KAT_SUCCESS); mlen++) {
            for (long adlen = 0; adlen <= MAX_ASSOCIATED_DATA_LENGTH; adlen++) {
                // count
                reader.readLine();
                // key
                reader.readLine();
                ...
                int[] ct = new int[msg.length + CRYPTO_ABYTES];
                long[] _clen = new long[msg.length + CRYPTO_ABYTES];

                if ((func_ret = PhotonBeetle128AEAD.crypto_aead_encrypt(
                ...
                if (! auth_CT.equals(myCTHex)) {
                   System.out.println("Author's CT and our CT are not equal: " + auth_CT + " " + myCTHex);

                } 
                

                long[] _mlen2 = new long[msg.length];
                _mlen2[0] = msg.length;
                if ((func_ret = PhotonBeetle128AEAD.crypto_aead_decrypt(
                ...
                if (! auth_PT.equals(myPTHex)) {
                   System.out.println("Author's CT and our CT are not equal: " + auth_PT + " " + myPTHex);
