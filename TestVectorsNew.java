...
class TestVectors {
    ...
    // Write the file name
    private static String file_name = "ELEPHANT-TEST-VEC.txt";
        ...
        try {
            ...
            for (int mlen = 0; mlen <= MAX_MESSAGE_LENGTH; mlen++) {
                for (int adlen = 0; adlen <= MAX_ASSOCIATED_DATA_LENGTH; adlen++) {
                    ...
                    int[] ct = new int[mlen + 10];
                    int[] clen = new int[2];
                    clen[0] = ct.length;
                    //int[] ct = RomulusT.crypto_aead_encrypt(msg, ad, nonce, key);
                    elephant.cryptoAeadEncrypt(ct, clen, msg, mlen, ad, adlen, nsec, npub, key);

                    myWriter.write("CT = " + convertToHexString(ct) + "\n");
                    
                    int[] dct = new int[mlen];
                    elephant.cryptoAeadDecrypt(dct, mlen, ct, clen[0], ad, adlen, nsec, npub, key);
